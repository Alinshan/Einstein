...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                ms = await client.send_cached_media(
                    chat_id=CH_FILTER,
                    file_id=file_id,
                    caption=f'<b>Hey 👋 {query.from_user.mention} 😍\n\n📂 File Name</b> : <code>{title}</code>\n\n<b>⚙️ File Size : {size}</b>\n\n<i>⚠️ This Message Will Be Auto-Deleted In Next 5 Minutes T𝘰 Avoid Copyright Issues.So Forward This File To Anywhere Before Downloading...⚠️</i>\n\n<b>🔮 Group : {query.message.chat.title}</b>',
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer(f"Hey, {query.from_user.first_name}! I Like Your Smartness, But Don't Be Oversmart 😌....Please Join The Channel To Get Movie Files 📂",show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        buttons= [[
            ...
            InlineKeyboardButton('📝 sᴏᴜʀᴄᴇ', url='https://t.me/Einstein_Source_Code')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        if str(grp_id) != str(grpid):
            await query.message.edit("Your Active Connection Has Been Changed. Go To /settings In Pm Chat And Edit Them.")
            return 
        ...
        if settings is not None:
            buttons = [
                ...
                [
                    InlineKeyboardButton('Files Send', callback_data=f'setgs#botpm#{settings["botpm"]}#{str(grp_id)}'),
